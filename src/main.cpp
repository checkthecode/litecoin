...
uint256 hashGenesisBlock("0xc6b3001bf2fc745cb85e64269412f4d2a8ab663cc3ff6209dc250199322924e2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Greececoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Greececoin Signed Message:\n";
...
{
    // Greececoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan =  60 * 60; // Greececoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Greececoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Greececoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Greececoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Greececoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc6b3001bf2fc745cb85e64269412f4d2a8ab663cc3ff6209dc250199322924e2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Country Coins for Country Toys";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x75238f8bfbc795783e8f8a4465f1edf81ee830b5fbfece30016992f38f2e5d48"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xe0, 0xd6, 0xdb }; // Greececoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GreececoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GreececoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GreececoinMiner(CWallet *pwallet)
{
    printf("GreececoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("greececoin-miner");
    ...
    try { loop {
        ...
        printf("Running GreececoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GreececoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GreececoinMiner, pwallet));
}
